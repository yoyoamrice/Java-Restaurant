...
// import org.springframework.samples.petclinic.store.repository.ProductRepository;
...
// @ServiceConnection
// @Container
// static MySQLContainer container = new
// MySQLContainer(DockerImageName.parse("mysql:9.5"));
//
// @LocalServerPort
// int port;
//
...
// RestTemplate template = builder.rootUri("http://localhost:" + port).build();
// ResponseEntity<String> result =
// template.exchange(RequestEntity.get("/schools/1").build(), String.class);
// assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
...
// RestTemplate template = builder.rootUri("http://localhost:" + port).build();
// ResponseEntity<String> result =
// template.exchange(RequestEntity.get("/products/1").build(), String.class);
// assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
